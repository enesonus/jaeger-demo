...
func main() {
	...
	server.HandleFunc("/album_artist", func(w http.ResponseWriter, r *http.Request) {
		...
		for _, album := range models.AlbumsSeed {
			if album.ID == id {
				if err := encoder.Encode(map[string]string{"artist": album.Artist}); err != nil {
